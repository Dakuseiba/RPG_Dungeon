...
public class MapPointList : MonoBehaviour
{
    ...
    /*public static List<int> IdPoints(int startedIdPoint, int destinyIdPoint)
    {
        List<int> idPoints = new List<int>();
        var points = StaticValues.points[startedIdPoint].MapPoint.NeighborPointID;
        var wanted = StaticValues.points[destinyIdPoint].MapPoint.idPoint;
        idPoints.Add(startedIdPoint);
        foreach (var point in points)
        {
            if (wanted == point.ID)
            {
                idPoints.Add(destinyIdPoint);
                return idPoints;
            }
            else
            {
                var result = IdPoints(StaticValues.points[point.ID], StaticValues.points[wanted], idPoints, StaticValues.points[startedIdPoint]);
                if (result != null)
                {
                    return result;
                }
    ...
    static List<int> IdPoints(MapPointController Point, MapPointController wanted, List<int> idPoints, MapPointController previous)
    {
        var points = Point.MapPoint.NeighborPointID;
        foreach (var point in points)
        {
            if (StaticValues.points[point.ID] != previous)
            {
                if (StaticValues.points[point.ID] == wanted)
                {
                    idPoints.Add(point.ID);
                }
                else
                {
                    IdPoints(StaticValues.points[point.ID], wanted, idPoints, Point);
                }
            }
        }
        return idPoints;
